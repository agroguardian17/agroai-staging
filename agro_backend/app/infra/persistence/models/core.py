...
otp_challenges, auth_sessions.

``otp_codes`` / ``refresh_tokens`` are the legacy (0002) auth tables; the
active auth flow uses ``otp_challenges`` / ``auth_sessions`` (0009). Both pairs
are modelled here so ``Base.metadata`` matches the migrated schema."""
...
class OtpChallenge(Base):
    """Active OTP-challenge store (migration 0009). Supersedes ``otp_codes``."""
...
class AuthSession(Base):
    """Active refresh-session store (migration 0009). Supersedes ``refresh_tokens``."""
