...
IMMUTABLE = {
    ...
    # --- D14 (Satellite & Remote Sensing) — prohibited customer claims (RAW MASTER §16.1, §5)
...
class OverrideStore:
        ...
        **payload,
        ...
        if kind == "THRESHOLD":
            ...
            if str(t["from"]) not in expr:
                raise OverrideRefused(
                    f"मूल्य {t['from']} या नियमाच्या अटीत सापडत नाही. override प्रत्यक्ष अटीशी जुळले पाहिजे."
                )
        ...
        ov = Override(
            ...
            created=day,
            expires=day + timedelta(days=expiry_days),
        ...
        self._log(
            "CREATED", rule_id, expert_id, f"{kind} scope={scope}:{scope_id} expires={ov.expires}"
        )
        return ov
    ...
    def effective(self, rule_id, day: date, plot_id=None, cluster_id=None) -> dict:
        ...
        applicable = [
            o
            for o in self.items
            if o.rule_id == rule_id and o.in_force(day) and o.applies_to(plot_id, cluster_id)
        ]
        ...
        for ov in applicable:
            ...
            if ov.kind == "DISABLE":
                ...
                out["expr"] = re.sub(
                    rf"\b{re.escape(str(ov.new_threshold['from']))}\b",
                    str(ov.new_threshold["to"]),
                    out["expr"],
                    count=1,
                )
    ...
    for rid, kind, _why, kw in [
        ...
        try:
            st.create(
                ...
                "AG-001",
                "डॉ. कदम",
                "या भागात हे लागू होत नाही असे वाटते.",
                scope="global",
                day=today,
                **kw,
            )
