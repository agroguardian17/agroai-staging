...
PRECEDENCE = [
    ...
    Relation(
        ...
        "SUPERSEDES",
        "D05-CH-003",
        "One pre-harvest interval block, not three. Duplication group pre_harvest_interval_block.",
...
class Precedence:
    ...
    def resolve(self, fired: list[Fired], answered: set[str] | None = None) -> Decision:
        ...
        for i in range(len(anchors) - 1):
            ...
            typed = any(
                r.object == b.rule_id and r.subject == a.rule_id
...
YELLOWING = {
    "soft_rot": dict(
        ...
        supports=[
            ("rhizome_smell", "sour_foul", 0.35),
            ("rhizome_texture", "mushy_wet", 0.30),
            ("soil_moisture_saturated", True, 0.15),
            ("shoot_pulls_out_easily", True, 0.15),
        ],
    ...
    "bacterial_wilt": dict(
        ...
        supports=[
            ("wilt_while_green", True, 0.40),
            ("stem_ooze_type", "milky_yellowish", 0.35),
            ("stem_cut_colour", "greyish_yellow", 0.20),
            ("field_history_wilt", True, 0.10),
        ],
        excludes=[],
    ...
    "fusarium": dict(
        ...
        decisive=[],
        supports=[
            ("rhizome_texture", "dry_rot", 0.45),
            ("stem_cut_colour", "brown_vascular", 0.30),
        ],
        excludes=[("ooze_test_result", "milky_thread")],
        next_test_mr="गड्डा कोरडा कुजला आहे की पचपचीत?",
    ),
    "nitrogen_deficiency": dict(
        name_mr="नत्राची कमतरता",
        rule="D04-NS-001",
        decisive=[],
    ...
    "iron_zinc_lockup": dict(
        name_mr="लोह/जस्त अनुपलब्धता",
        rule="D04-DG-002",
        decisive=[],
        supports=[
            ("leaf_yellowing_pattern", "interveinal_new", 0.55),
            ("soil_free_lime_present", True, 0.30),
        ],
    ...
    "heat_stress": dict(
        name_mr="उष्णतेची इजा",
        rule="D07-HS-002",
        decisive=[],
        supports=[
            ("air_temp_above_35_3d", True, 0.45),
            ("leaf_yellowing_pattern", "margin_scorch", 0.35),
...
def diagnose(observations: dict, model=YELLOWING) -> dict:
    ...
    if state == "CONFIRMED":
        ...
        msg = f"बहुधा {top.name_mr} ({int(top.confidence * 100)}%). खात्रीसाठी: {top.next_test_mr}"
    else:
        names = " किंवा ".join(c.name_mr for c in cands[:3])
        msg = f"दोन शक्यता आहेत — {names}. उपचार करण्यापूर्वी वेगळे करा. {top.next_test_mr}"
...
if __name__ == "__main__":
    ...
    d = p.resolve(
        [
            Fired("D03-WR-001", "info", 5),
            Fired("D03-MN-002", "red", 5),
            Fired("D03-WL-001", "red", 5, 0.35),
            Fired("D03-WL-002", "red", 5),
    ...
    d = p.resolve(
        [
            Fired("D08-EU-001", "yellow", 5, 0.125),
            Fired("D04-NS-002", "yellow", 5),
            Fired("D08-MU-002", "yellow", 4),
            Fired("D08-EU-005", "yellow", 5),
        ]
    )
    print(f"  दाखवले   : {d.issued}")
    ...
    cases = [
        ...
        (
            ...
            {
                "leaf_yellowing_pattern": "margin_scorch",
                "air_temp_above_35_3d": True,
