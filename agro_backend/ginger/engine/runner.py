...
ENGINE_VERB = re.compile(r"(इशारा द्या|सूचना द्या|यादी द्या|मांडा|दाखवा|नोंदवा आणि)\b")
...
class Runner:
    def __init__(self):
        ...
        self.compiled = {
            rid: parse(spec["expr"], self.allowed) for rid, spec in TRIGGERS.items() if rid in rules
        }
        ...
        *,
        ...
        for rid, node in self.compiled.items():
            ...
            if r.outcome == TV.TRUE:
                ...
                fired.append(
                    Fired(rid, sev, self.rules[rid]["priority"], self.rules[rid].get("u_value"))
                )
                if eff["overrides"]:
        ...
        obs = {
            ...
            for k in [
                ...
                "soil_free_lime_present",
                "air_temp_above_35_3d",
...
def render_day(res, ctx, show_internals=False) -> str:
    ...
    if show_internals:
        out.append("  ── अंतर्गत ──")
        out.append(
            f"     फिरले {len(res['fired'])} · दिले {len(res['messages'])} · "
...
def demo_context(kind="earthing_day"):
    ...
    base = dict(
        soil_type="vertisol",
        soil_texture_class="heavy",
        soil_depth_cm=45,
        percolation_time_hours=9,
        drainage_outlet_present=True,
        variety="mahima",
        has_drip=True,
        planting_layout="broad_ridge",
        area_acre=1.0,
        ceiling_quintal_per_acre=113,
        ceiling_basis="variety_plus_broad_ridge_113",
        yield_target_quintal_per_acre=90,
        seasonal_water_requirement_litres=2_700_000,
        water_available_oct_feb_litres=2_900_000,
        season_water_plan_basis="poor_year",
        drainage_levels_present=3,
        main_drain_connected=True,
        moisture_probe_depth_cm=12,
        soil_test_available=True,
        calibration_done=True,
        vwc_saturation=45.0,
        vwc_field_capacity=38.0,
        vwc_stress_threshold=22.0,
        consent_advisory=True,
        consent_research=True,
        cluster_anonymised=True,
        seed_retained_or_purchased="purchased",
        field_history_rot=False,
        field_history_wilt=False,
        intercrop_selected="marigold",
        pre_sanction_received=True,
        subsidy_scheme_applied="drip",
        breakeven_price_per_quintal=1450,
        plants_per_acre=24300,
        seed_piece_weight_g=35,
        cost_seed=40000,
        drying_method="simple_sun",
        ppe_available=True,
        processing_trained_operator=True,
        seed_supplier_identified=True,
        deep_ploughing_done=True,
        kulav_passes=4,
        solarization_done=True,
        mulch_stage_1_done=True,
        emergence_started=True,
        establishment_pct=88.0,
        castor_bait_prepared_date="done",
        light_trap_installed=True,
        micronutrient_spray_1_done=True,
        pest_scouting_date="done",
        pmfby_notified_for_ginger="no",
    )
    ...
    if kind == "earthing_day":
        base.update(
            ...
            rainfall_mm=2.0,
            rh_pct=78.0,
            soil_moisture_vwc=34.0,
            soil_moisture_vwc__duration=0,
            rh_pct__duration=24,
            forecast_rain_48h_mm=3.0,
            rain_gap_days=1,
            dry_spell_days=1,
            days_to_harvest=153,
            flowering_observed=False,
            tillers_per_plant=12.0,
            labour_arranged_date="set",
        ...
        base.update(
            ...
            rainfall_mm=48.0,
            rh_pct=93.0,
            soil_moisture_vwc=48.0,
            soil_moisture_vwc__duration=30,
            rh_pct__duration=96,
            forecast_rain_48h_mm=35.0,
            rain_gap_days=0,
            dry_spell_days=0,
            days_to_harvest=140,
            earthing_up_date="done",
            n_split_2_date="done",
            mulch_stage_2_done=True,
            water_stress_after_earthing_done=True,
            micronutrient_spray_2_done=True,
            flowering_observed=False,
            tillers_per_plant=12.0,
            central_shoot_dead=True,
            rhizome_smell="sour_foul",
        ...
        base.update(
            ...
            dry_spell_days=9,
            days_to_harvest=185,
            micronutrient_spray_1_done=False,
            flowering_observed=False,
            planting_date="2026-06-05",
        )
        ...
        base.update(
            ...
            rainfall_mm=0.0,
            rh_pct=34.0,
            deep_ploughing_done=False,
            solarization_done=False,
            mulch_stage_1_done=False,
            emergence_started=False,
            establishment_pct=None,
            castor_bait_prepared_date=None,
            light_trap_installed=False,
            micronutrient_spray_1_done=False,
            seed_supplier_identified=False,
            pest_scouting_date=None,
            pmfby_notified_for_ginger="unverified",
            current_stage="G0",
            vafsa_state="workable",
            kulav_passes=0,
        )
        ...
        base.update(
            ...
            days_to_harvest=60,
            earthing_up_date="done",
            earthing_up_2_date="done",
            n_split_2_date="done",
            k_late_split_1_date="done",
            k_late_split_2_date="done",
            mulch_stage_2_done=True,
            mulch_stage_3_done=True,
            micronutrient_spray_2_done=True,
            water_stress_after_earthing_done=True,
            flowering_observed=True,
            sample_dig_120_done=True,
            planting_date="2026-06-05",
        )
    return base
...
if __name__ == "__main__":
    ...
    cases = (
        ["preseason", "earthing_day", "heat_spray", "saturation", "quiet"]
        if a.case == "all"
        else [a.case]
    )
    LABEL = {
        "preseason": "हंगामापूर्व — नांगरट बाकी",
