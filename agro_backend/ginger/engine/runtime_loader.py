    ...
    def load(self) -> dict:
        ...
        for fn in self.FILES:
            ...
            if g:
                precedence = [
                    Relation(
                        x["subject"], x["relation"], x["object"], x["reason_en"], x["reason_mr"]
                    )
                    for x in g
                ]
...
def _shape(d: dict) -> dict:
    ...
    return dict(
        ...
        immutable=bool(d.get("immutable")),
        immutable_reason=d.get("immutable_reason"),
        trigger=dict(english=d["trigger_en"], marathi=d["trigger_mr"], expr=d.get("trigger_expr")),
        action=dict(english=d["action_en"], marathi=d["action_mr"]),
        reasoning=dict(
            agronomic_basis=d["agronomic_basis"],
...
def _assemble(rules, fields, precedence) -> dict:
    triggers = {rid: r["trigger"]["expr"] for rid, r in rules.items() if r["trigger"].get("expr")}
    delivery = {rid: r["delivery"] for rid, r in rules.items() if r.get("delivery")}
    immutable = {
        rid: (r.get("immutable_reason") or "immutable")
...
if __name__ == "__main__":
    ...
    a = Runner()
    print(f"  नियम {len(a.rules)} · triggers {len(a.compiled)} · संबंध {len(a.prec.by_sub)} विषय")

    print("\n═══ knowledge base मधून वाचल्यास ═══\n")
    b = build_runner(JsonSource())
    print(f"  नियम {len(b.rules)} · triggers {len(b.compiled)} · संबंध {len(b.prec.by_sub)} विषय")
