...
_ROT = UValue(factor_key="soft_rot", u_value=0.60, signal_field="rot_incidence_pct",
              representative_rule_id="D06-ROT-001", rank=1)
_WILT = UValue(factor_key="bacterial_wilt", u_value=0.50, signal_field="wilt_incidence_pct",
               representative_rule_id="D06-WILT-001", rank=3)


def test_no_signals_predicts_the_ceiling_with_no_loss() -> None:
    p = predict_yield(
        ceiling_basis="variety_only_94", factors=[_ROT, _WILT],
        signals={}, prediction_stage="mid_season",
    )
    ...
    assert p.prediction_interval_pct == 20.0  # mid_season
...
def test_rot_signal_applies_its_u_value() -> None:
    p = predict_yield(
        ceiling_basis="variety_only_94", factors=[_ROT, _WILT],
        signals={"rot_incidence_pct": 50.0}, prediction_stage="g1_end",
    )
    ...
    assert p.prediction_interval_pct == 22.0  # g1_end
...
def test_broad_ridge_layout_raises_the_ceiling() -> None:
    ...
    p = predict_yield(
        ceiling_basis="variety_plus_broad_ridge_113", factors=[_ROT],
        signals={}, prediction_stage="pre_season",
    )
    assert p.ceiling_quintal_per_acre == CEILING_QTL_PER_ACRE["variety_plus_broad_ridge_113"]
    assert p.prediction_interval_pct == 25.0  # pre_season


def test_ceiling_override_wins_for_arithmetic() -> None:
    p = predict_yield(
        ceiling_basis="variety_only_94", factors=[_ROT],
        signals={"rot_incidence_pct": 100.0}, prediction_stage="mid_season",
