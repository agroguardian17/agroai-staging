...
class Override:
    ...
    kind: str
    scope: str
    scope_id: Optional[str]          # plot_id / cluster_id, None for global
    expert_id: str
    expert_name: str
    rationale_mr: str
    created: date
    expires: date
    # payload, one of:
    new_threshold: Optional[dict] = None      # {'from': 12, 'to': 9}
    new_delivery: Optional[str] = None
    new_severity: Optional[str] = None
    new_parameter: Optional[dict] = None      # {'name':'cost_mulch','from':15000,'to':11000}
    disable_reason_mr: Optional[str] = None
    revoked: Optional[date] = None
    ...
    def applies_to(self, plot_id=None, cluster_id=None):
        if self.scope == 'global': return True
        if self.scope == 'plot': return self.scope_id == plot_id
        if self.scope == 'cluster': return self.scope_id == cluster_id
        return False
    ...
    def revoke(self, override_id, expert_id, day=None):
        ...
        for ov in self.items:
            if ov.override_id == override_id and ov.active:
                ov.revoked = day; ov.revoked_by = expert_id
...
if __name__ == '__main__':
    ...
    for label, fn in [
        ...
        try:
            fn(); print(f"  ❌ {label} स्वीकारला — चुकीचे")
        except OverrideRefused as e:
