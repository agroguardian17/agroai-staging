...
from precedence import Precedence, Fired, diagnose
from notification_policy import Notifier, SILENT_GUARD
from expert_override import OverrideStore, load_rules
...
SEV_ICON = {'blocking': '⛔', 'red': '🔴', 'yellow': '🟡', 'info': 'ℹ️'}
...
ENGINE_TAIL = re.compile(
    r'\s*(कारण सांगा[:：]?|हे वापरकर्त्याला सांगा\.?|स्पष्ट सांगा[:：]?|'
...
def render_day(res, ctx, show_internals=False) -> str:
    ...
    if show_internals:
        ...
        if res['suppressed']:
            out.append(f"     दडपले: " + ", ".join(f"{a}←{b}" for a, b in res['suppressed'][:4]))
        if res['unknown']:
