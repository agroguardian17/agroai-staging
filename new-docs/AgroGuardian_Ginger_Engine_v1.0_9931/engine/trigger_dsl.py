...
class TV:
    ...
    @staticmethod
    def and_(a, b):
        if a == TV.FALSE or b == TV.FALSE: return TV.FALSE
        if a == TV.UNKNOWN or b == TV.UNKNOWN: return TV.UNKNOWN
        return TV.TRUE

    @staticmethod
    def or_(a, b):
        if a == TV.TRUE or b == TV.TRUE: return TV.TRUE
        if a == TV.UNKNOWN or b == TV.UNKNOWN: return TV.UNKNOWN
        return TV.FALSE

    @staticmethod
    def not_(a):
        if a == TV.TRUE: return TV.FALSE
        if a == TV.FALSE: return TV.TRUE
        return TV.UNKNOWN
...
class Node:
    def fields(self) -> set[str]: return set()
    def eval(self, ctx: dict, trace: list) -> str: raise NotImplementedError
    def to_str(self) -> str: raise NotImplementedError


@dataclass
class Cmp(Node):
    fld: str; op: str; val: Any
    def fields(self): return {self.fld} | ({self.val} if isinstance(self.val, FieldRef) and False else set())
    def eval(self, ctx, trace):
        ...
        try:
            ...
            trace.append((self.to_str(), TV.UNKNOWN, "type mismatch")); return TV.UNKNOWN
...
class InSet(Node):
    fld: str; vals: list
    def fields(self): return {self.fld}
    def eval(self, ctx, trace):
        v = ctx.get(self.fld)
        if v is None:
            trace.append((self.to_str(), TV.UNKNOWN, f"{self.fld}=None")); return TV.UNKNOWN
        out = TV.TRUE if v in self.vals else TV.FALSE
        trace.append((self.to_str(), out, f"{self.fld}={v!r}")); return out
    def to_str(self):
...
class Between(Node):
    ...
    def eval(self, ctx, trace):
        ...
        trace.append((self.to_str(), out, f"{self.fld}={v}")); return out
    def to_str(self): return f"{self.fld} BETWEEN {self.lo} AND {self.hi}"


@dataclass
class IsCheck(Node):
    fld: str; what: str          # NULL | NOT_NULL | TRUE | FALSE
    def fields(self): return {self.fld}
    def eval(self, ctx, trace):
        present = self.fld in ctx and ctx[self.fld] is not None
        v = ctx.get(self.fld)
        if self.what == 'NULL':      out = TV.TRUE if not present else TV.FALSE
        elif self.what == 'NOT_NULL': out = TV.TRUE if present else TV.FALSE
        elif not present:             out = TV.UNKNOWN
        elif self.what == 'TRUE':     out = TV.TRUE if v is True else TV.FALSE
        else:                         out = TV.TRUE if v is False else TV.FALSE
        trace.append((self.to_str(), out, f"{self.fld}={v!r}")); return out
    def to_str(self):
...
class Duration(Node):
    ...
    fld: str; inner_op: str; inner_val: Any; n: Any; unit: str
    def fields(self): return {self.fld, f"{self.fld}__duration"}
    def eval(self, ctx, trace):
        key = f"{self.fld}__duration"
        d = ctx.get(key)
        if d is None:
            trace.append((self.to_str(), TV.UNKNOWN, f"{key} not supplied")); return TV.UNKNOWN
        out = TV.TRUE if d > self.n else TV.FALSE
        trace.append((self.to_str(), out, f"{key}={d}")); return out
    def to_str(self):
...
class Within(Node):
    ...
    fld: str; n: Any; unit: str
    def _key(self):
        base = self.fld.replace('_date', '')
        return f"days_to_{base}"
    def fields(self): return {self.fld, self._key()}
    def eval(self, ctx, trace):
        d = ctx.get(self._key())
        if d is None:
        ...
        return out
    def to_str(self): return f"WITHIN({self.fld}, {self.n} {self.unit})"


@dataclass
class Not(Node):
    inner: Node
    def fields(self): return self.inner.fields()
    def eval(self, ctx, trace): return TV.not_(self.inner.eval(ctx, trace))
    def to_str(self): return f"NOT ({self.inner.to_str()})"


@dataclass
class And(Node):
    parts: list
    def fields(self): return set().union(*(p.fields() for p in self.parts))
    def eval(self, ctx, trace):
        out = TV.TRUE
        for p in self.parts: out = TV.and_(out, p.eval(ctx, trace))
        return out
    def to_str(self): return " AND ".join(f"({p.to_str()})" if isinstance(p, Or) else p.to_str()
                                          for p in self.parts)
...
class Or(Node):
    ...
    def eval(self, ctx, trace):
        out = TV.FALSE
        for p in self.parts: out = TV.or_(out, p.eval(ctx, trace))
        return out
    def to_str(self): return " OR ".join(p.to_str() for p in self.parts)
...
class Parser:
    def __init__(self, toks, known_fields=None):
        self.t = toks; self.i = 0; self.known = known_fields

    def peek(self): return self.t[self.i] if self.i < len(self.t) else None
    def next(self):
        t = self.peek()
        if t is None: raise SyntaxError("unexpected end of expression")
        self.i += 1; return t
    ...
    def expect(self, kind, val=None):
        ...
        if not t: raise SyntaxError(f"expected {val or kind} at token {self.i}")
        return t

    def parse(self):
        n = self.or_expr()
        if self.peek(): raise SyntaxError(f"trailing tokens from position {self.peek().pos}")
        return n

    def or_expr(self):
        parts = [self.and_expr()]
        while self.accept('KW', 'OR'): parts.append(self.and_expr())
        return parts[0] if len(parts) == 1 else Or(parts)

    def and_expr(self):
        parts = [self.not_expr()]
        while self.accept('KW', 'AND'): parts.append(self.not_expr())
        return parts[0] if len(parts) == 1 else And(parts)

    def not_expr(self):
        if self.accept('KW', 'NOT'): return Not(self.not_expr())
        return self.primary()

    def primary(self):
        if self.accept('(', '('):
            n = self.or_expr(); self.expect(')', ')'); return n

        t = self.peek()
        if t and t.kind == 'KW' and t.val == 'DURATION':
            self.next(); self.expect('(', '(')
            fld = self.field_name(); op = self.expect('OP').val; iv = self.value()
            self.expect(')', ')'); self.expect('OP', '>'); n = self.value()
            unit = self.expect('KW').val
            return Duration(fld, op, iv, n, unit)

        if t and t.kind == 'KW' and t.val == 'WITHIN':
            self.next(); self.expect('(', '(')
            fld = self.field_name(); self.expect(',', ',')
            n = self.value(); unit = self.expect('KW').val
            self.expect(')', ')')
            return Within(fld, n, unit)

        if t and t.kind == 'KW' and t.val in ('MONTH', 'STAGE'):
            kw = self.next().val
            self.expect('KW', 'IN'); vals = self.value_list()
        ...
        if self.accept('KW', 'BETWEEN'):
            lo = self.value(); self.expect('KW', 'AND'); hi = self.value()
            return Between(fld, lo, hi)
        if self.accept('KW', 'IS'):
            if self.accept('KW', 'NOT'):
                self.expect('KW', 'NULL'); return IsCheck(fld, 'NOT_NULL')
    ...
    def value(self):
        t = self.next()
        if t.kind == 'NUM': return t.val
        if t.kind == 'STR': return t.val
        if t.kind == 'KW' and t.val in ('TRUE', 'FALSE'): return t.val == 'TRUE'
        if t.kind == 'FIELD':
            if self.known is not None and t.val not in self.known:
    ...
    def value_list(self):
        self.expect('[', '[')
        vals = [self.value()]
        while self.accept(',', ','): vals.append(self.value())
        self.expect(']', ']')
...
class Result:
    ...
    @property
    def fired(self): return self.outcome == TV.TRUE
...
if __name__ == '__main__':
    ...
    for e, ctx in tests:
        n = parse(e, known); r = evaluate(n, ctx)
